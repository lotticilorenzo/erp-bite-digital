...
async def test_validation():
    ...
    result_data = {"success": False, "error": None, "details": None}
    ...
    async with async_session() as session:
        try:
            ...
            if not c:
                ...
                # Simulate a PATCH with a Drive URL
                update_data = ClienteUpdate(google_drive_url="https://drive.google.com/test")
                for field, val in update_data.model_dump(exclude_none=True).items():
                    setattr(c, field, val)
                
                try:
                    out = ClienteOut.model_validate(c, from_attributes=True)
                    result_data["success"] = True
                except ValidationError as ve:
                    result_data["error"] = "Validation Error"
                    result_data["details"] = ve.errors()
