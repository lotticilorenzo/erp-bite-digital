...
class FatturaAttiva(Base):
    ...
    fattura_elettronica: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)  # flag SDI (§10.1)
...
class FatturaPassiva(Base):
    ...
    fattura_elettronica: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)  # flag SDI (§10.1)
    data_ricezione_fe: Mapped[Optional[date]] = mapped_column(Date)
    detraibilita_pct: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2))  # IVA detraibile % (da FIC, §6.2)
    deducibilita_pct: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2))  # costo deducibile %
    valuta: Mapped[Optional[str]] = mapped_column(String(10))
    categoria: Mapped[Optional[str]] = mapped_column(String(100))
    payments_raw: Mapped[Optional[dict]] = mapped_column(JSON)
    fic_raw_data: Mapped[Optional[dict]] = mapped_column(JSON)
...
class MovimentoCassa(Base):
    ...
    imponibile: Mapped[Optional[Decimal]] = mapped_column(Numeric(14, 2))  # da FIC (§5.1)
