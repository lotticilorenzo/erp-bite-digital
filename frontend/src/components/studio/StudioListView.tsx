...
export function StudioListView() {
  ...
  return (
    <div className="flex-1 overflow-auto bg-[#020617]/50">
      <Table>
        <TableHeader className="bg-[#0f172a]/80 sticky top-0 z-10 backdrop-blur-md border-b border-[#1e293b]/50">
        ...
        <TableBody>
          ...
          <TableRow className="hover:bg-white/5 border-[#1e293b]/20 group">
...
function TaskRow({ task, depth = 0 }: { task: TaskSO | SubtaskSO; depth: number }) {
  ...
  return (
    <>
      <TableRow 
        className={`hover:bg-white/5 border-[#1e293b]/10 group cursor-pointer transition-colors ${isTimerActive ? 'bg-primary/5 shadow-inner' : ''}`}
      ...
      >
          ...
          <div className="flex items-center gap-2" style={{ paddingLeft: `${depth * 24}px` }}>
            {hasSubtasks ? (
              <button 
                ...
                className="p-1 rounded-md hover:bg-white/10 text-[#475569] hover:text-white transition-all"
              >
            ...
            <div className={`h-2.5 w-2.5 rounded-full shrink-0 shadow-[0_0_8px_rgba(124,58,237,0.3)] transition-colors ${isTimerActive ? 'bg-primary animate-pulse' : 'bg-[#1e293b]'}`} />
            <span className={`text-[13px] font-bold truncate ${isTimerActive ? 'text-white' : 'text-[#94a3b8]'}`}>
        ...
        <TableCell>
          <div className="flex -space-x-2">
            {[1].map((i) => (
              <div key={i} className="h-6 w-6 rounded-full border border-[#020617] bg-[#1e293b] flex items-center justify-center text-[10px] font-bold text-white shadow-lg">
        ...
        <TableCell>
          <Badge variant="outline" className="text-[10px] font-black uppercase bg-transparent border-[#1e293b] text-[#475569]">
