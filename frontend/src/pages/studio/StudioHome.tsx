    ...
    <div className="p-8 pb-12 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground font-medium">Hai {data?.length} attività che richiedono la tua attenzione.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button size="lg" className="rounded-xl font-black uppercase tracking-widest px-8 shadow-[0_0_20px_hsl(var(--primary)/0.2)] hover:scale-105 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <Card key={stat.title} className="bg-card/40 border-border/50 backdrop-blur-xl group hover:border-primary/50 transition-all duration-500 overflow-hidden relative">
              ...
              <div className="divide-y divide-[#1e293b]/30">
                {data?.slice(0, 5).map((task) => (
                  <div key={task.id} className="p-6 flex items-center gap-4 hover:bg-white/5 transition-colors cursor-pointer group">
                    <div className="h-10 w-10 shrink-0 rounded-xl bg-muted/50 border border-border/50 flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                      <Zap className="h-5 w-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="text-sm font-bold text-white truncate">{task.title}</h4>
                        <Badge variant="outline" className={`text-[10px] font-black uppercase bg-transparent border-border ${
                          task.state_id === 'closed' ? 'text-emerald-400 border-emerald-400/20' : 'text-primary border-primary/20'
                        }`}>
                          {task.state_id}
                        </Badge>
                      </div>
                      <p className="text-xs text-muted-foreground truncate">Folder ID: {task.folder_id}</p>
                    </div>
        ...
        <div className="space-y-8">
          <Card className="bg-gradient-to-br from-primary/10 to-transparent border-primary/20 backdrop-blur-xl rounded-3xl overflow-hidden p-6 relative group">
            ...
            <div className="relative z-10 space-y-4">
              <div className="h-12 w-12 rounded-2xl bg-primary flex items-center justify-center shadow-lg shadow-[0_0_20px_hsl(var(--primary)/0.2)]">
                <AlertCircle className="h-6 w-6 text-white" />
              </div>
              <div>
                <h3 className="text-lg font-black text-white leading-tight">Focus del Giorno</h3>
                <p className="text-xs text-muted-foreground mt-1 font-medium italic">"Non contare i giorni, fa che i giorni contino."</p>
              </div>
              <div className="pt-2">
          ...
          <Card className="bg-card/40 border-border/50 backdrop-blur-xl rounded-3xl p-6">
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex gap-3 relative pb-4 after:absolute after:left-[7px] after:top-[22px] after:bottom-0 after:w-[2px] after:bg-muted/30 last:after:hidden last:pb-0">
                  <div className="h-4 w-4 mt-1 rounded-full bg-muted shrink-0 border-2 border-[#000]" />
                  <div>
